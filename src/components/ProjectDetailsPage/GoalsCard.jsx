  ...
  return (
    <section className="bg-white rounded-lg shadow p-4 sm:p-6">
      ...
      {(!project.goals || project.goals.length === 0) ? (
        ...
        <section className="max-h-[300px] overflow-y-auto pr-2" aria-label="Goals list">
          <ul className="space-y-3">
            {project.goals.map((goal, index) => (
              <li key={index}>
                <article className="flex items-start gap-3 p-2 bg-gray-50 rounded-lg group">
                  <input
                    type="checkbox"
                    ...
                    className="mt-1"
                  />
                  <p className={`${goal.completed ? 'line-through text-gray-500' : ''} flex-1`}>
                    {goal.text}
                  </p>
                  {isProjectOwner(project) && (
                    <button
            ...
            <div className="flex min-h-screen items-center justify-center p-4">
              ...
              >
                ...
                <form onSubmit={handleAddGoal}>
                  <label htmlFor="goalInput" className="sr-only">Goal description</label>
                  <input
                    id="goalInput"
                    type="text"
                    value={newGoal}
